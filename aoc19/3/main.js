...
wires.forEach((wire, index) => {
  ...
  wire.forEach(point => {
    var dir = point[0];
    var mag = Number(point.split(dir).pop());
    var dx = 0;
    var dy = 0;
    ...
    for (i=0; i < Math.abs(mag); i++) {
      cx += dx;
      cy += dy;
      steps++;

      if (index == 0) {
        map.push({x:cx, y:cy, steps:steps})
      } else {
        for(var x=0; x<map.length; x++){
          point = map[x];
          if (point.x == cx && point.y == cy) {
            distances.push(steps+point.steps)
...
// intersections.forEach(sect => {
//  distances.push(Math.abs(sect.x) + Math.abs(sect.y))
// })
